...
class TaskOffloadingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if processing_info is not None:
            reward = self._calculate_reward(task, processing_info)
        else:
            reward = -2  # Set a large negative reward for unprocessable tasks
    ...
    def _calculate_reward(self, task, processing_info):
        ...
        if resource_delay > task_acceptable_delay:
            reward = -2  # Unacceptable delay
        else:
            delay_factor = (task_acceptable_delay - resource_delay) / task_acceptable_delay
            cost_factor = task_complexity * resource_cost
            distance_factor = 1 - (resource_distance / 100)  # Normalize distance to a factor

            reward = delay_factor + cost_factor + distance_factor
...
def display_table(resources, resource_type):
    if resource_type == "Task":
        ...
        rows = [[f"{resource_type} {idx + 1}", task.cpu_cores, task.min_acceptable_delay, task.task_complexity] for
                idx, task in enumerate(resources)]
    else:
        headers = ["Resource", "Cores", "Delay", "Cost", "Distance"]
        rows = []
        for idx, resource in enumerate(resources):
            rows.append([f"{resource_type} {idx + 1}", resource['cores'], resource['delay'], resource['cost'],
                         resource['distance']])
...
def train_a2c(env, num_episodes, gamma, lr):
    ...
    plt.savefig('episode_rewards_VTO_a2c_v1.png')
    plt.show()  # Keep the plot window open until it's closed by the user
...
if __name__ == "__main__":
    ...
    lr = 0.001  # Learning rate for the A2C network
    num_episodes = 100  # Traning episodes
