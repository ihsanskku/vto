#VTO with DoubleDQN v2. (with successfull/unsuccessful task number and grpagh only)
import os
...
def train_double_dqn(env, agent, num_episodes, batch_size, epsilon_decay):
    ...
    for episode in range(num_episodes):
        ...
        print(f"Episode {episode}: Total Reward = {episode_reward}, Successful Tasks = {episode_successful}, Unsuccessful Tasks = {episode_unsuccessful}")

    return episode_rewards, episode_successful_tasks, episode_unsuccessful_tasks
...
if __name__ == "__main__":
    ...
    episode_rewards, episode_successful_tasks, episode_unsuccessful_tasks = train_double_dqn(env, agent, num_episodes, batch_size, epsilon_decay)
    ...
    with open('episode_results.csv', 'w', newline='') as csvfile:
        ...
        for episode, (reward, successful, unsuccessful) in enumerate(
                zip(episode_rewards, episode_successful_tasks, episode_unsuccessful_tasks)):
    ...
    plt.savefig('episode_rewards_VTO_DDQN_v2.png')
    plt.show()
    ...
    plt.savefig('Task_succ_unsucc_VTO_DDQN_v2.png')
    plt.legend()
    plt.show()
