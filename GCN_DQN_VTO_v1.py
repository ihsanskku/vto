...
# Define the classes
class Task:
    def __init__(self, task_id, cpu_cores, data_size, bandwidth_requirements, min_acceptable_delay):
...
def generate_tasks(num_tasks):
    ...
    for i in range(num_tasks):
        ...
        tasks.append(Task(i, cpu_cores, data_size,bandwidth_requirements, min_acceptable_delay))
    return tasks
...
def generate_devices(num_vehicles, num_rsus, num_cloud_servers):
    vehicles = [Vehicle(cpu_cores=random.randint(100, 200), bandwidth=random.randint(100, 101),
                        ...
                        distance=random.randint(100, 200)) for _ in range(num_vehicles)]
    rsus = [RoadSideUnit(cpu_cores=random.randint(300, 400), bandwidth=random.randint(200, 201),
                         ...
                         distance=random.randint(100, 200)) for _ in range(num_rsus)]
    cloud_servers = [CloudServer(cpu_cores=random.randint(800, 900), bandwidth=random.randint(200, 201),
                                 ...
                                 distance=random.randint(300, 301)) for _ in range(num_cloud_servers)]
...
class TaskAllocationEnv(gym.Env):
    def __init__(self, num_tasks, num_vehicles, num_rsus, num_cloud_servers):
        ...
        # Initialize network
        self.G, self.data = process_network(self.vehicles, self.rsus, self.cloud_servers)
    ...
    def reset(self):
        ...
        # Reset tasks
        self.tasks = generate_tasks(len(self.tasks))

        # Reset devices
        self.vehicles, self.rsus, self.cloud_servers = generate_devices(len(self.vehicles), len(self.rsus),
                                                                        len(self.cloud_servers))

        # Update network based on the reset devices
        self.G, self.data = process_network(self.vehicles, self.rsus, self.cloud_servers)
