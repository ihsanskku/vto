#VTO with DoubleDQN v6. (with reward, successfull/unsuccessful tasks , and episode cost graph, extended code with validation)....>> task comlexity function + bandwith
...
def generate_tasks(num_tasks):
    ...
    for i in range(num_tasks):
        ...
        min_acceptable_delay = random.randint(15, 20) # 5 to 15 second delay
...
def create_resources(num_vehicles, num_rsus, num_cloud_servers):
    vehicles = [Vehicle(i, random.randint(5, 10), random.randint(100, 500), i) for i in range(num_vehicles)]
    road_side_units = [RoadSideUnit(i, random.randint(25, 30), random.randint(100, 500)) for i in range(num_rsus)]
    cloud_servers = [CloudServer(i, random.randint(100, 500), random.randint(1000, 2000)) for i in range(num_cloud_servers)]
...
class TaskOffloadingEnv(gym.Env):
    def __init__(self, num_tasks, num_vehicles, num_rsus, num_cloud_servers):
        ...
        self.resource_availability = {
            'vehicles': [{'cores': vehicle.cpu_cores, 'delay': (2 + 1), 'cost': 0.15, 'distance': random.randint(10, 100), #(2 + 1) computation dealy +  nework latency
                          'bandwidth': random.randint(100, 500)}  # Assign bandwidth to vehicles
                         for vehicle in self.vehicles],
            'road_side_units': [
                {'cores': road_side_unit.cpu_cores, 'delay': (0.3 + 5), 'cost': 0.25, 'distance': random.randint(10, 100), #(0.3 + 5) computation dealy +  nework latency
                 'bandwidth': random.randint(100, 500)}  # Assign bandwidth to RSUs
                for road_side_unit in self.road_side_units],
            'cloud_servers': [
                {'cores': cloud_server.cpu_cores, 'delay': (0.1 + 10), 'cost': 0.35, 'distance':100,  #(0.1 + 10) computation dealy + nework latency
    ...
    def _process_task(self, task, resource_type):
        ...
        for idx, resource in enumerate(resource_list):
            if task.cpu_cores <= resource['cores']:
                ...
                resource_id = idx  # Use the index of the resource as resource_id
                return {
...
def train_double_dqn(env, agent, num_episodes, batch_size, epsilon_decay, validation_interval, num_validation_episodes):
    ...
    for episode in range(num_episodes):
        ...
        print(f"Episode {episode + 1}: Total Reward = {round(episode_reward, 2)}, Successful Tasks = {episode_successful}, Unsuccessful Tasks = {episode_unsuccessful}, Episode Cost = {round(episode_cumulative_cost,2)}")
...
if __name__ == "__main__":
    ...
    num_validation_episodes = 200  # Number of episodes for validation
    ...
    plt.savefig('episode_rewards_VTO_DDQN_v6_10000.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('Successful_and_Unsuccessful_Tasks_VTO_DDQN_v6_10000.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('episode_cumulative_cost_VTO_DDQN_v6_10000.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('Average Validation Rewards_VTO_DDQN_v6_10000.png')
    plt.show()