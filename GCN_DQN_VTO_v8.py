#State contain task and nodes combine task and nodes in state V3
...
def generate_devices(num_vehicles, num_rsus, num_cloud_servers):
    vehicles = [Vehicle(cpu_cores=random.randint(100, 200), bandwidth=random.randint(100, 101),
                        ...
                        distance= random.randint(50, 200)) for _ in range(num_vehicles)]
...
def process_network(vehicles, rsus, cloud_servers, tasks, current_task_idx, graph_idx):
    # Create a graph
    G = nx.Graph()
    ...
    #for task in tasks:
        ...
        G.add_node(task, type="Task", cpu_cores=task.cpu_cores, bandwidth_requirements=task.bandwidth_requirements, data_size=task.data_size, min_acceptable_delay=task.min_acceptable_delay)
...
class TaskAllocationEnv(gym.Env):
    def __init__(self, num_tasks, num_vehicles, num_rsus, num_cloud_servers):
        ...
        # Initialize network
        self.graph_idx = 0
        self.G, self.data = process_network(self.vehicles, self.rsus, self.cloud_servers, self.tasks, self.current_task_idx, self.graph_idx)
    ...
    def step(self, action):
        print("Selected node\n", self.data.x[action])
        #print("Before the action", self.data.x)
        ...
        #self.G, self.data = process_network(self.vehicles, self.rsus, self.cloud_servers, self.tasks,self.current_task_idx, self.graph_idx)
        #plot_network(env.G)
        #task_values = tf.constant([task.cpu_cores, task.data_size, task.bandwidth_requirements, task.min_acceptable_delay, 0.0], dtype=tf.float32)
        #self.data.x = tf.concat([self.data.x[:-1, :], task_values], axis=0)
        ...
        if done != True:
            # task = self.tasks[self.current_task_idx]
            # self.data.x[-1][0] = task.cpu_cores
            # self.data.x[-1][1] = task.bandwidth_requirements
            # self.data.x[-1][2] = task.data_size
            # self.data.x[-1][3] = task.min_acceptable_delay
            # self.data.x[-1][4] = 0.0
            self.G, self.data = process_network(self.vehicles, self.rsus, self.cloud_servers, self.tasks,
                                                self.current_task_idx, self.graph_idx)
    ...
    def _calculate_reward(self, task):
        ...
        if (self.data.x[action][0] >= task.cpu_cores and
            self.data.x[action][1] >= task.bandwidth_requirements and
            self.data.x[action][4] <= 150): # nieghbour vehicle, rsu, and cloud server must be less or equal to 100m distance
