#VTO with DoubleDQN v3. (with reward, successfull/unsuccessful tasks , and episode cost graph)
...
class TaskOffloadingEnv(gym.Env):
    def __init__(self, num_tasks, num_vehicles, num_edge_servers, num_cloud_servers):
        ...
        self.vehicles, self.edge_servers, self.cloud_servers = create_resources(num_vehicles, num_edge_servers,
                                                                                num_cloud_servers)
    ...
    def reset(self):
        # Reset the environment (tasks and resources) to initial state and return the initial observation
        self.tasks = generate_tasks(num_tasks)
        self.current_task_idx = 0
        self.resource_availability = {
            'vehicles': [{'cores': vehicle.cpu_cores, 'delay': 0.1, 'cost': 0.15, 'distance': random.randint(10, 100)}
...
# Training Loop
def train_double_dqn(env, agent, num_episodes, batch_size, epsilon_decay):
    epsilon = 1.0
    epsilon_min = 0.01
    episode_rewards = []
    ...
    for episode in range(num_episodes):
        ...
        print(
            f"Episode {episode}: Total Reward = {episode_reward}, Successful Tasks = {episode_successful}, Unsuccessful Tasks = {episode_unsuccessful}, Episode Cost = {episode_cumulative_cost}")

    return episode_rewards, episode_cumulative_costs, episode_successful_tasks, episode_unsuccessful_tasks
...
if __name__ == "__main__":
    ...
    episode_rewards, episode_cumulative_costs, episode_successful_tasks, episode_unsuccessful_tasks = train_double_dqn(
        env, agent, num_episodes, batch_size, epsilon_decay)

    # Saving episode rewards, successful tasks, unsuccessful tasks, and cumulative costs to a CSV file
    with open('episode_results.csv', 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['Episode', 'Reward', 'Successful Tasks', 'Unsuccessful Tasks', 'Cumulative Cost'])
        for episode, (reward, successful, unsuccessful, cumulative_cost) in enumerate(
                zip(episode_rewards, episode_successful_tasks, episode_unsuccessful_tasks, episode_cumulative_costs)):
            csv_writer.writerow([episode, reward, successful, unsuccessful, cumulative_cost])
    ...
    plt.savefig('episode_rewards_VTO_DDQN_v3.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('Successful_and_Unsuccessful_Tasks_VTO_DDQN_v3.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig('episode_cumulative_cost_VTO_DDQN_v3.png')
    plt.show()
